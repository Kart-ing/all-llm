...
**OpenAI-compatible API that never 429s. Bring your OpenRouter key, rotate across every free model automatically.**
...
2. For each incoming request it builds a rotation order: your preferred model first, then round-robin through the rest, skipping any models on cooldown.
...
4. For streaming requests we **only rotate on the initial HTTP status**. Once SSE bytes start flowing we can't safely switch mid-stream — the client has already committed to a conversation. Non-streaming requests are the happy path for retries.
...
**Pick always-llm if** you already have an OpenRouter key and you just want the 429s to stop.
...
Identical to OpenAI's endpoint. The `model` field is a *preference*, not a requirement — rotation may substitute a different free model. Response includes an `x-always-llm-model` header showing which model served the request.
...
- **v0.1** (you are here): in-memory cooldown, single region.
